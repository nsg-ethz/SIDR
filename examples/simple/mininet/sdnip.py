...
class BgpRouter(Router):
    ...
    def __init__(self, name, intfDict, asNum, neighbors, routes=[], quaggaConfFile=None,
                 zebraConfFile=None, runDir='/var/run/quagga', *args, **kwargs):
    ...
    def generateQuagga(self):
        ...
        for neighbor in self.neighbors:
            ...
            if 'local_pref' in neighbor:
                route_mape_name = 'as' + str(neighbor['as']) + '-in'
                writeLine(1, 'neighbor %s route-map %s in' % (neighbor['address'], route_mape_name))
            writeLine(1, '!')
        ...
        for neighbor in self.neighbors:
            if 'local_pref' in neighbor:
                writeLine(0, '!')
                writeLine(0, 'route-map %s permit 5' % (route_mape_name))
                writeLine(0, ' set local-preference %s' % (neighbor['local_pref']))
                writeLine(0, '!')
